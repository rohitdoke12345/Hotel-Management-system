...
import Mess_Services.customerService;
import Mess_Services.foodService;
import Mess_Services.roomService;
...
//                    ArrayList <customerModel> regCust = cs.getallcustomer(); 
				if (regCust.isEmpty()) {
					...
					for (customerModel cust : regCust) {
						System.out.println(cust.getCid() + "\t" + cust.getName() + "\t" + cust.getEmail() + "\t"
								+ cust.getPhonenumber());
					}
				}
				break;
